...
from app.controllers.auth import login_for_access_token, refresh_access_token, logout_with_token
...
async def logout(
        ...
        db_session: Annotated[AsyncSession, Depends(get_async_postgres_session)]):
    result = await logout_with_token(request=request, response=response, db_session=db_session)
    return result
