...
@tests_router.get("/test-runs/{run_id}", response_model=TestRunResponse, status_code=200)
async def get_test_run(run_id: int, current_user: User = Depends(get_user_from_token),
                       async_db_session: AsyncSession = Depends(get_async_postgres_session)):
